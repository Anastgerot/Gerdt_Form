...
{
    wstring commandLine = L"\"" + path + L"\" " + to_wstring(id);
    STARTUPINFO si = { sizeof(si) };
    PROCESS_INFORMATION pi;
    if (CreateProcess(NULL, &commandLine[0], NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi))
    {
        CloseHandle(pi.hThread);
        CloseHandle(pi.hProcess);
        wcout << L"Клиент запущен: " << commandLine << endl;
    }
    else
    {
    ...
    while (true) {
        try
        {
            ...
            case MT_INIT:
            {
                ...
                wcout << "Client connect" << std::endl;
                sendData(s, &session->id, sizeof(int));
                broadcastUpdate();
                break;
            }
            case MT_EXIT:
            ...
            case MT_UPDATE: {
                ...
                //response += L'\0';
                wcout << "update: \"" << response << "\"" << std::endl;

                int dataSize = static_cast<int>(response.size() * sizeof(wchar_t));

                std::wcout << L"Длина response  (wchar_t): " << response.size() << std::endl;
                std::wcout << L"Длина response byte: " << dataSize << std::endl;
...
int main() {
    try {
        ...
        while (true) {
            ...
            a.accept(socket);
            thread(processClient, std::move(socket)).detach();
        }
    }
